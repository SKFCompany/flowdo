...
# ═══════════════════════════════════════════════════════════════════════════
#  АВТОУСТАНОВКА зависимостей для голосового помощника
#  Определяем платформу и устанавливаем нужные пакеты автоматически.
#  Android: используем android.tts (встроенный) и SpeechRecognizer
#  Windows/Linux/macOS: pyttsx3 для TTS, vosk+pyaudio для STT
# ═══════════════════════════════════════════════════════════════════════════
import sys as _sys
import subprocess as _subprocess

def _detect_platform():
    """Определяет платформу: 'android', 'windows', 'linux', 'macos'"""
    try:
        import android  # noqa
        return "android"
    except ImportError:
...
# ───────────────────────────────────────────────────────────────────────────
#  Вспомогательная функция — MDLabel с поддержкой emoji
#  Если EMOJI_FONT найден — использует markup и font-тег.
#  Иначе — обычный MDLabel (emoji могут отображаться квадратами).
# ───────────────────────────────────────────────────────────────────────────
def _wrap_emoji_only(text):
    """Оборачивает в [font=EMOJI_FONT] только сами emoji-символы,
    оставляя кириллицу и латиницу в дефолтном шрифте KivyMD."""
    if not EMOJI_FONT:
        return text.replace("&","&amp;").replace("[","&#91;").replace("]","&#93;"), False
    import re
    # Диапазоны Unicode emoji (основные блоки)
    EMOJI_RE = re.compile(
        "[\U0001F000-\U0001FFFF"   # основной блок emoji (💖🌸🔴 и др.)
        "\U00002600-\U000027FF"    # разные символы (✨☀ и др.)
        "\U00002300-\U000023FF"    # технические символы
        "\U00002B00-\U00002BFF"    # ⭐ и другие стрелки/символы
        "\U00002900-\U000029FF"    # дополнительные стрелки
        "\U00003000-\U000030FF"    # CJK символы с emoji
        "\U0000FE00-\U0000FE0F"    # вариационные селекторы
        "\U0001F900-\U0001F9FF"    # дополнительные emoji
        "\U00002702-\U000027B0"    # орнаменты
        "\U00002500-\U000025FF"    # блочные символы
        "]+",
        re.UNICODE
    )
    result = []
    ...
    for m in EMOJI_RE.finditer(text):
        # Обычный текст до emoji — экранируем и добавляем как есть
        plain = text[last:m.start()]
        if plain:
            result.append(plain.replace("&","&amp;").replace("[","&#91;").replace("]","&#93;"))
        # Сам emoji — оборачиваем в emoji-шрифт
        em = m.group().replace("&","&amp;").replace("[","&#91;").replace("]","&#93;")
        result.append(f"[font={EMOJI_FONT}]{em}[/font]")
        has_emoji = True
        last = m.end()
    # Хвост
    tail = text[last:]
    ...
    return "".join(result), has_emoji


def EmojiLabel(text="", font_style="Body1", **kwargs):
    """MDLabel с поддержкой emoji: только emoji-символы рендерятся
    через emoji-шрифт, кириллица остаётся в дефолтном шрифте."""
    markup_text, has_emoji = _wrap_emoji_only(text)
    if has_emoji and EMOJI_FONT:
        lbl = MDLabel(text=markup_text, font_style=font_style, markup=True, **kwargs)
    else:
...
def update_emoji_label(lbl, new_text):
    """Обновляет текст EmojiLabel с сохранением emoji-рендеринга."""
    lbl._emoji_text = new_text
    markup_text, has_emoji = _wrap_emoji_only(new_text)
    if has_emoji and EMOJI_FONT:
        lbl.markup = True
        lbl.text = markup_text
    else:
        lbl.markup = False
        lbl.text = new_text
    lbl._use_emoji_font = has_emoji and bool(EMOJI_FONT)
...
class DailyTodoApp(MDApp):
    ...
    def _mk_cat_btn(self, cat):
        ...
        lbl=EmojiLabel(text=txt, font_style="Caption", bold=sel,
                    halign="center", valign="middle",
    ...
    def _mk_stats_page(self):
        ...
        def _mini(ico_txt, title, sub, attr):
            box=MDBoxLayout(orientation="vertical", spacing=S(2), padding=[S(14),S(10)])
            ...
            ico=EmojiLabel(text=ico_txt, font_style="H5",
                           size_hint_y=None, height=S(30),
                           halign="left", valign="middle")
            ...
            val_l=MDLabel(text="0", font_style="H5", bold=True,
                          theme_text_color="Primary", size_hint_y=None, height=S(28),
                          halign="left", valign="middle")
        ...
        mood_box=MDBoxLayout(orientation="vertical", size_hint_y=None, height=S(160),
                             spacing=S(6), padding=[S(16),S(12)])
        ...
        hist_r=MDBoxLayout(orientation="horizontal", size_hint_y=None, height=S(32), spacing=S(4))
